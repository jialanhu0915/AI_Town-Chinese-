    ...
    try:
        ...
        for agent_id, agent in world.agents.items():
            try:
                ...
                print(f'   - {agent.name}: {action.get("type", "unknown")} - {action.get("description", action.get("reason", ""))}')
        ...
        for step in range(3):
            ...
            active_actions = sum(1 for result in step_results.values() 
                               if result.get('type') != 'idle')
            print(f'   第 {step + 1} 步: {active_actions} 个活跃行动，{len(world.current_events)} 个事件')
        ...
        class TestAgent(BaseAgent):
            def __init__(self):
                super().__init__(
                    ...
                    name="TestAgent",
                    age=25,
                    personality={'extraversion': 0.5, 'agreeableness': 0.5, 'conscientiousness': 0.5, 
                               'neuroticism': 0.5, 'openness': 0.5},
