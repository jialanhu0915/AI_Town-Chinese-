...
async def test_llm_agents():
    ...
    for agent in agents:
        try:
            # 添加一些测试记忆
            test_memories = [
                Observation(GameTime.now(), agent.agent_id, "social", "与朋友聊天", Position(0, 0, "test")),
                Observation(GameTime.now(), agent.agent_id, "work", "在咖啡店工作", Position(0, 0, "test")),
                Observation(GameTime.now(), agent.agent_id, "exploration", "探索新地方", Position(0, 0, "test"))
            ]
