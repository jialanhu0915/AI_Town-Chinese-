...
class ActionPlanner:
    ...
    def _initialize_basic_goals(self):
        """初始化基本的生活目标"""
        basic_goals = [
            Goal(id="maintain_energy", description="Keep energy levels above 30%", priority=9.0),
            Goal(
    ...
    async def plan_next_action(
        self, world_state: Dict[str, Any], context_memories: List[Any], current_state: Any
    ...
    def _assess_needs(
        self, world_state: Dict[str, Any], context_memories: List[Any]
    ) -> Dict[str, float]:
        """评估智能体的当前需求"""
        needs = {"energy": 0.0, "social": 0.0, "work": 0.0, "exploration": 0.0, "maintenance": 0.0}
        ...
        recent_social_memories = [
            ...
            if any(
                keyword in memory.description.lower()
    ...
    async def _get_or_create_plan(
        self, goal: Goal, world_state: Dict[str, Any], context_memories: List[Any]
    ) -> Optional[Plan]:
        """获取或创建实现目标的计划"""
        # 查找现有的活跃计划
        existing_plan = next(
            (plan for plan in self.plans if plan.goal_id == goal.id and plan.status == "active"),
            None,
        )
    ...
    async def _create_maintenance_plan(self, world_state: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if self.agent.energy < 40:
            ...
            actions.append(
                {"type": "sleep", "duration": 30, "description": "Take a restorative nap"}
            )
        else:
            # 正常维护活动
            actions.append(
    ...
    def _generate_default_action(
        self, world_state: Dict[str, Any], needs: Dict[str, float]
    ) -> Dict[str, Any]:
        """生成默认行动"""
        # 根据最高需求生成行动
