...
class GameMap:
        ...
        for x in range(self.width):
            for y in range(self.height):
                self.tiles[(x, y)] = MapTile(
                    x=x, y=y, 
                    terrain_type=TerrainType.WALKABLE,
                    area_name="outdoors"
                )
    ...
    def _create_buildings(self):
        ...
        buildings_config = [
            # 住宅区
            {"id": "house_1", "name": "Alice's House", "type": "home", 
             "x": 5, "y": 5, "w": 8, "h": 6},
            {"id": "house_2", "name": "Bob's House", "type": "home", 
             "x": 5, "y": 15, "w": 8, "h": 6},
            {"id": "house_3", "name": "Charlie's House", "type": "home", 
             "x": 5, "y": 25, "w": 8, "h": 6},
            
            # 商业区
            {"id": "coffee_shop", "name": "Alice's Coffee Shop", "type": "shop", 
             "x": 20, "y": 20, "w": 10, "h": 8},
            {"id": "bookstore", "name": "The Book Corner", "type": "shop", 
             "x": 35, "y": 20, "w": 8, "h": 6},
            {"id": "grocery", "name": "Town Grocery", "type": "shop", 
             "x": 20, "y": 35, "w": 12, "h": 8},
            
            # 办公区
            {"id": "office_1", "name": "Town Office", "type": "office", 
             "x": 60, "y": 30, "w": 15, "h": 10},
            {"id": "library", "name": "Public Library", "type": "office", 
             "x": 60, "y": 15, "w": 12, "h": 8},
            
            # 娱乐设施
            {"id": "park", "name": "Central Park", "type": "park", 
             "x": 45, "y": 45, "w": 20, "h": 15},
            {"id": "restaurant", "name": "Town Restaurant", "type": "restaurant", 
             "x": 25, "y": 50, "w": 10, "h": 8}
        ]
    ...
    def get_nearby_agents(self, center_x: float, center_y: float, radius: float, agent_positions: Dict[str, Any]) -> List[Dict[str, Any]]:
        """获取指定范围内的其他智能体"""
        nearby = []
    ...
    def get_buildings_in_area(self, area_name: str) -> List[Building]:
        ...
        return [building for building in self.buildings.values() 
                if any(self.get_tile(building.entrance_x + dx, building.entrance_y + dy) and
                      self.get_tile(building.entrance_x + dx, building.entrance_y + dy).area_name == area_name
                      for dx in range(building.width) for dy in range(building.height))]
