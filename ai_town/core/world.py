...
class World:
    ...
    def add_agent(self, agent: BaseAgent):
        ...
        # 在地图上注册智能体位置
        building = self.map.get_building_at(
    ...
    async def _process_talk(self, agent_id: str, action: Dict[str, Any]):
        ...
        # 发送消息给目标智能体
        target.receive_message(agent_id, message, {
            'sender_name': speaker.name,
            'location': speaker.position.area
        })
    ...
    async def _process_interactions(self):
        ...
        for i, agent1 in enumerate(agent_list):
            for agent2 in agent_list[i+1:]:
                ...
                if (distance <= 2.0 and 
                    agent1.state.value in ['idle', 'socializing'] and
                    agent2.state.value in ['idle', 'socializing']):
    ...
    async def _create_automatic_interaction(self, agent1: BaseAgent, agent2: BaseAgent):
        ...
        # 发送消息
        agent2.receive_message(agent1.agent_id, message, {
